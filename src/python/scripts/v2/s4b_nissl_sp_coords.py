"""
Script to map bead positions to coordinate space of original nissl images.
Initial generation for sharing with Partha lab.

Usage:

python s4b_nissl_sp_coords.py \
    inp: nissl_id \
...
python src/python/scripts/v2/s4b_nissl_sp_coords.py \
    -1 \
    ...
    /Users/mraj/Desktop/work/data/mouse_atlas/data_v3_nissl_post_qc/s4_bead_to_segid/bead_to_nis_coords

Created by Mukund on 2022-12-06
...
for nissl_id in nissl_ids:
    ...
    output_csv_file = f'{output_csv_folder}/bead_to_nis_coords_{nis_idx}.csv'
    # write output file
    with open(output_csv_file, 'w', newline='\n') as csvfile:
        ...
        for idx, row in enumerate(labels):
            ...
            dom_status = "IN"
            if mapper_to_id[row] == "OUTDOM":
                dom_status = "OUT"
            line = [dom_status, nissl_img_corrds[idx][0], nissl_img_corrds[idx][1], width, height]
