"""
Generates data for heatmap plot in viewer. Run after first calling
s9_qc_labelgen.py and calls s9c_qc_prep.R from within script.

Usage:
python s9c_qc_main.py \
    inp: data root
    inp: path to aggregated labels files generated by s9_qc_labelgen.py
    inp: path to gene counts data
    i/o: path to save intermediate aggregated data generated by called R script
    out: (redundant-to be removed) path to store output files to be read by viewer
    inp: gene list comma separated (currently must by at least of length 2)
    inp: puck range - start and end values (inclusive) separated by space
    inp: boolean option to indicate whether to only consider gene count nonzero (currently always False)
    inp: region ids - based on regions set in s9_qc_labelgen.py
    out: path to store output files to be read by viewer
